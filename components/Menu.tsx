...
export default function Menu({
  ...
  return (
    <div>
      <div
        className="fixed flex top-0 left-0 w-full h-full bg-transparent z-100 cursor-pointer"
        ...
        <div className="flex flex-col items-center justify-center space-y-4">
          {menuItems.map((item) => (
            <Link key={item.title} href={item.href}>
              <button
                key={item.title}
                className="text-white text-xl underline-offset-10 underline cursor-pointer"
                onClick={() => callback(false)}
              >
                {item.title}
              </button>
